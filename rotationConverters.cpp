...
void calib::Euler(const Mat& src, Mat& dst, int argType) {
...
void calib::RodriguesToEuler(const Mat& src, Mat& dst, int argType) {
...
void calib::EulerToRodrigues(const Mat& src, Mat& dst, int argType) {
	CV_Assert((src.cols == 1 && src.rows == 3) || (src.cols == 3 && src.rows == 1));
	Mat R;
	Euler(src, R, argType);
	Rodrigues(R, dst);
